...
### 23. Commit verification as a test; a command you ran once defends nothing
**Symptom** An epic shipped six slugs whose acceptance was checked by hand — fixture branches seeded on a sandbox repository, both halves of a rule table diffed against each other, five extraction cases, a commit-idempotence check. Every one passed. Every one then evaporated: the fixtures were deleted, the scratch repositories were `/tmp`, and the repository ended the day with **no** `tests/` directory and no `make test`. A later round could reintroduce any of those defects and nothing would notice.
**Cause** An agent run is a measurement, not a guard. It proves the state of one revision at one moment, and it leaves no artifact that the next change must satisfy. Acceptance criteria written as prose invite exactly this: the model runs the command, reports the output, and the knowledge dies with the transcript.
**Rule** When a behaviour is worth verifying, commit an executable test for it and wire it into the project's own test target. Prefer a hermetic test with its I/O boundaries stubbed over one that needs the network, so it runs on every change rather than when someone remembers. A card's acceptance item should name the test it adds; a one-off command belongs in a test file, not in a worklog. Report a defect you cannot fix as a failing-with-reason test, never as a paragraph.
**Proven** 2026-08-09/10: rule 4b's bash and Python halves were proven identical by hand against seven live fixture branches, which were then deleted, leaving the parity of two money-spending code paths defended by nothing at all. The repository grew `tests/run`, `tests/lib.sh` and its first cases the next morning; the parity check became a hermetic case with both HTTP boundaries stubbed, and a real defect that is not yet fixed became a TODO test that turns red the moment it is.
...
### 21. Pass a check its configuration by absolute path outside your own checkout, or it passes vacuously
**Symptom** `make scrub` in a `git worktree add --detach` scratch tree exited `0` and printed `ob-scrub-check: no deny list at <wt>/.scrub-deny; nothing to check.` The identical command with `OPENBUILDER_SCRUB_DENY` pointed at the real list also exited `0`, printing `ob-scrub-check: clean (history).` Two runs, same status, opposite meanings: one checked seven patterns, the other checked nothing.
**Cause** The deny list is gitignored deliberately, so it exists only in the operator's own checkout. A fresh worktree is a fresh checkout and inherits nothing git does not track. Any configuration a check reads from an untracked file is absent there by construction — not occasionally, but always, on every machine.
**Rule** Before running a check anywhere but the operator's own tree, pass its configuration in by absolute path and assert the tool reports having done the work — `clean`, a count, a named artifact. Never accept an exit code from a check whose skip path also exits `0`. When a check can be vacuous, refuse explicitly: `refusing to treat a skipped check as a pass`.
**Proven** 2026-08-09, while designing auto-merge, whose whole purpose is to substitute for the CI this repository does not have. Measured before the code was written, so the vacuous version never shipped.

### 22. Assert a tool's literal output with `grep -F`, never a bare pattern
**Symptom** `grep -c "^{{PRD}}$" runner/prompts/implement.md` printed `0` for a file that contains exactly that line, and `grep -c "docs(epic): PRD and RFC for" docs/architecture.md` printed `0` for a file that contains exactly that text. Both patterns are correct under a basic-regex grep. The same session, on a different machine, an implementation round hit the identical failure and wrote it in its worklog.
**Cause** Under an extended-regex grep, `{` and `}` are interval-expression metacharacters and `(` and `)` are group metacharacters, so a pattern copied from expected output stops being a literal. Basic-regex grep treats all four as ordinary characters, so the same pattern works on one host and silently fails on another. Nothing in the failure distinguishes "pattern did not match" from "text is absent".
**Rule** Assert literal output with `grep -F` (or `grep -qF`). Reserve bare patterns for genuine regexes. This matters most in acceptance criteria and in gate checks: a detector that fails to match reports a clean pass or refuses correct work, and in both cases it looks like the check working.
**Proven** 2026-08-09, twice independently: an implementation round on the build instance recorded it in `plan-workflow-03-context`'s worklog, and it cost the reviewer two false measurements while verifying that same pull request. Both were re-run with `-F` and matched. Proposed by the round that earned it; committed by the reviewer.

### 24. A heredoc ends its `&&` chain; the line after `EOF` runs unconditionally
**Symptom** A single shell invocation shaped like `TOKEN=$(mint) && cat > /tmp/body.md <<EOF ... EOF` followed on the next line by `GH_TOKEN="$TOKEN" gh pr merge ...` merged a pull request with an EMPTY token after the mint had failed. The `cat` was correctly skipped by the short-circuit, so the body file never existed — but the merge on the following line ran anyway, silently falling back to the ambient credentials, and GitHub recorded the wrong actor.
**Cause** A heredoc terminator closes the command it belongs to. Everything after `EOF` is a new command separated by a newline, not part of the preceding `&&` chain, so the chain's short-circuit protects only up to `EOF`. An empty variable then reaches the next command as an empty environment value rather than as an error, and tools that accept ambient credentials treat that as "use the default".
**Rule** Never continue an `&&` chain across a heredoc. Write the file in its own command and re-check the precondition on the next line — `[[ -n $TOKEN ]] || exit 1` — before any command that mutates something. For anything irreversible, assert the credential is present and is the one you intended, because a fallback to ambient auth looks exactly like success.
**Proven** 2026-08-10: an expired AWS session made the App-token mint fail; the merge ran regardless and was attributed to the operator instead of the bot. The exit status of the whole invocation was non-zero, which is precisely why it was not noticed until the attribution was read back.
