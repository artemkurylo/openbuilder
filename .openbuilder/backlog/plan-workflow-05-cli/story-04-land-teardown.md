...
acceptance:
  ...
  - "after a successful land in a sandbox, all three of `openbuilder/work/<slug>`, `openbuilder/plan/<slug>` and `openbuilder/design/<epic>` are absent from `gh api repos/<repo>/git/matching-refs/heads/openbuilder/`"
  - "when `state.json.slugs` still lists a slug whose plan branch exists, the design branch survives and stdout contains `openbuilder dispatch <repo> <that-slug>`"
---
...
9. Decide the design branch. List the surviving plan refs with one call,
   `ob_gh api "repos/$repo/git/matching-refs/heads/$OB_BRANCH_PREFIX/plan/" --jq '.[].ref'`, made
   **after** step 8 so this slug is no longer among them. A slug from `state.json.slugs` is *unlanded*
   when a ref `refs/heads/$OB_BRANCH_PREFIX/plan/<that-slug>` is still present; land deletes the plan
   branch, so its absence is the landed signal.
...
11. Final report on stdout: `landed <repo>#<pr> :: <slug>` with one line per deleted branch. When step
    9 kept the design branch, print the remaining slugs in `state.json.slugs` order, one dispatch
...
- **Sandbox required** for everything below. Set up: `gh repo create <you>/ob-sandbox --private` with
  ...
  3. **Epic with a slug left.** Push `openbuilder/plan/other-slug` from the design branch. Then
     `printf 'land land-slug\n' | local/bin/openbuilder land <you>/ob-sandbox <pr>` exits `0` or `6`
     (`6` when no instance is reachable, which is expected in a sandbox — assert the warning text
     ...
     - `gh api repos/<you>/ob-sandbox/git/matching-refs/heads/openbuilder/ --jq '.[].ref'` contains
       `refs/heads/openbuilder/design/land-epic` and `refs/heads/openbuilder/plan/other-slug`, and
       contains neither `openbuilder/plan/land-slug` nor `openbuilder/work/land-slug`;
     - stdout contains `openbuilder dispatch <you>/ob-sandbox other-slug`.
  4. **Last slug of the epic.** Repeat the setup with `slugs: ["only-slug"]` and one pull request.
     After a confirmed land,
     `gh api repos/<you>/ob-sandbox/git/matching-refs/heads/openbuilder/ --jq length` prints `0` — all
     three branches gone.
  5. **Instance prune, once, against the real instance.** With
